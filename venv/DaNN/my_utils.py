...
def vis_tSNE(input_data, input_label, classes, vis, name, n_dim=2):
    ...
    vis.scatter(X=da, Y=input_label, win=name,
                opts=dict(legend=legends, title='%s-tSNE(time %.2f s)' % (name, time() - t0)))
...
def plot_confusion_matrix(y_pre, y_true, disp_acc=False):
    """
    :param y_pre: (nc*nq, )
    :param y_true: (nc*nq, )
    :param disp_acc:
    ...
    if disp_acc:  # 归一化,可显示准确率accuracy,默认不显示准确率
        cm = cm.astype('float32') / cm.sum(axis=1)[:, np.newaxis]
    else:
        pass
    sns.heatmap(cm, annot=True, ax=ax, cmap='plasma')  # cmap如: plasma, viridis, magma, inferno
