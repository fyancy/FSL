...
class BasicModel(nn.Module):
    ...
    def get_encoded_inputs(self, *x_list):
        ...
        x_all = x_all.view(-1, self.chn, self.img_w)
    ...
    def kmeans_predict(self, n_class, y_s, x_s, x_u, x_q):
        """
        x_s: [B, Ns, 2048, 1]
        x_u: [B, Nu, 2048, 1]
        x_q: [B, Nq, 2048, 1]
...
class ssProto(BasicModel):
    ...
    def __init__(self, device):
        super(ssProto, self).__init__()
        self.d = device

    def forward(self, nc, x_s, x_u, x_q, cm=False):
        bsize = x_s.shape[0]
        nq, ns = x_q.shape[1], x_s.shape[1]
        assert nc == bsize
        y_q = torch.arange(0, nc).view(nc, 1, 1).expand(nc, nq, 1).long()  # [B, M, 1]
        y_s = torch.arange(0, nc).view(nc, 1, 1).expand(nc, ns, 1).long()
        y_s, y_q = y_s.to(self.d), y_q.to(self.d)
        logits = self.kmeans_predict(nc, y_s, x_s, x_u, x_q)  # [B, N, K]
        # log_p_y = torch.log_softmax(logits, dim=-1).view(bsize, nq, -1)  # [B, M, K]
        log_p_y = torch.log_softmax(logits, dim=-1)  # [B, M, K]
        # print('log_p_y\n', log_p_y.cpu().detach().numpy())
        ...
        if cm:
            ...
            y_true = y_q.contiguous().view(-1).squeeze().cpu().detach().numpy()
            plot_confusion_matrix(y_pre=y_pre, y_true=y_true, disp_acc=True)
