...
from Attention_Block import SELayer1
...
class Protonet(nn.Module):
    ...
    def forward(self, xs, xq, vis=None, sne_state=False):
        ...
        if sne_state:
            ...
            zs_data = z_proto.unsqueeze(dim=1).expand([n_class, n_support, z_dim])
            # [nc, z_dim]==>[nc, 1, z_dim]==>[nc, ns, z_dim]
            # 使用.expand似乎会造成存储空间不连续，类似于对tensor实施转置也会造成存储空间不连续
            # 此时，就不能使用.view()，解决方法是在.view()前增加.contiguous()
            zs_data = zs_data.contiguous().view(-1, z_dim).cpu().detach().numpy()
            zs_label = label
            vis_tSNE(zs_data, zs_label, classes=n_class, vis=vis, name='support centroids', n_dim=2)
